...
This is the single most-important paper finding from §8: **on cross-dataset evaluation, two of the four meaningfully-active auxiliary analyzers actively hurt**. The fuser's heuristic 0.1 weights for texture/moire (§5.4) saved them from this fate; `device_boundary` and `micro_tremor` carry capture-time priors that are silent failures on out-of-distribution data, while `background_grid` is the lone in-house-weighted analyzer that retains its discriminative value out-of-distribution. **Recommendation: re-derive the fuser weights per operator dataset with a proper Dev/Test split (§5.4), paying particular attention to the analyzers most sensitive to capture-time priors (bezel patterns, tripod tremor signatures).**

## 8.3 Heuristic vs. uniform analyzer weights (Table 6)

`MultiClassFuser` uses heuristic, hand-set weights (see `src/infrastructure/fusion/multi_class_fuser.py`; these are NOT a swept/optimised calibration — §5.4). All four configurations below are computed from a single full-hybrid per-analyzer capture on the in-house **synthetic** replay sub-protocol (N=83 reachable samples), re-fused offline under each weight map by `tests/benchmark/ablation_leave_one_out.py --mode weights`. **Caveat:** ACER/EER here use the EER-on-test operating point on the in-house synthetic set, so the absolute ACER/EER values are biased low and indicative only; treat this table as a *relative* comparison of weight maps on one fixed capture, not as a Dev→Test result. Because every row is computed from the *same* capture, the cross-row AUC/ACER/EER deltas are internally consistent (no cross-environment mixing); each row is backed by its own committed `weightcfg_in_house_replay_n100_<config>.json`, which embeds the full weight map, the per-sample analyzer scores, and the per-sample fused P(REAL), so every number here is recomputable offline.
...
| **Heuristic weights (paper default)** | 14.03% | 12.03% | 0.9424 | `weightcfg_…_calibrated.json` |
| MiniFASNet-dominant (5.0 / 0.1 others) | 15.76% | 16.62% | **0.9552** | `weightcfg_…_minifasnet_dominant.json` |

The heuristic-weights row is the full hybrid pipeline at its published default weights and is the offline re-fusion of the same baseline reported in the leave-one-out study (§8.2): identical ACER (14.03%) and EER (12.03%), N=83.
