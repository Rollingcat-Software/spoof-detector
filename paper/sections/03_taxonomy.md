...
- **Depth.** A depth sensor disambiguates PRINT (zero depth variance across the face surface) from MASK_3D (non-zero variance with characteristic mask geometry) from REAL (skin-surface depth profile). The fuser would extend with a depth analyzer routing strong evidence into PRINT (low depth) and MASK_3D (anomalous depth).
- **Infrared.** An IR sensor disambiguates REPLAY (the screen emits a different IR signature than skin) and AR_FILTER (the underlying skin still produces normal IR; the AR filter renders to RGB only and does not modify the IR channel). The fuser would extend with an IR-skin-signature analyzer routing strong evidence away from REAL when the IR signature is incompatible with skin.

Neither extension is exercised in §7 (RGB-only protocol for parity with the published pipeline). They are listed because the multi-class taxonomy is the natural shape for a depth- or IR-aware extension; a binary live/spoof fuser would need a structural redesign to absorb the new modalities, whereas the 7-class fuser absorbs them as additional analyzers in `src/infrastructure/fusion/multi_class_fuser.py:43-93` without changing the category structure.
