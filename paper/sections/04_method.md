...
Figure 1 summarises the engine. A single-camera RGB feed enters at 30 FPS. The face detector (MediaPipe Tasks API) yields bounding boxes and landmarks at ~2 ms per frame. A multi-target IoU tracker assigns persistent IDs so that temporal analyzers operate on the same face across frames. Each frame is then fanned out to two banks of analyzers operating in parallel. The Python reference evaluated in §6–§8 runs **13 analyzers total**: the MiniFASNet ONNX discriminator plus **twelve auxiliary analyzers** — four image-level (§4.1) and eight temporal (§4.2) — matching the thirteen analyzer modules in `src/infrastructure/analyzers/`. (The production browser bundle adds further net-new analyzers on top of these twelve auxiliaries; that larger count is scoped explicitly in §1 Contribution 1 and §10.1 and is not the configuration evaluated in §7–§8.)
...
### 4.5.1 Temporal flash-response probe (auto-brightness disclosure)

The colour-flash challenge above scores the *spatial* response of the flash (chromatic gain, region spread). We add a complementary *temporal* probe that exploits a behaviour absent from any static or spatial test: a replay screen's **auto-brightness control reacts to the flash and then latches**. With camera exposure locked to a mid-range value (so the sensor neither saturates nor auto-compensates), the screen is flashed white for ≈ 1.5 s while the face-region brightness is sampled at ≈ 10 Hz, followed by a post-flash sampling window. A genuine 3-D face is a passive reflector: brightness rises the instant the flash lands and returns to baseline the instant it ends. A phone/tablet replay whose ambient-light sensor sees the flash raises its own backlight over ≈ 1 s and holds it elevated after the flash ends.

The decision statistic is the **normalised post-flash persistence** $p = (\bar{B}_{\text{after}} - \bar{B}_{\text{base}}) / (B_{\text{peak}} - \bar{B}_{\text{base}})$, where the leading post-flash samples are discarded to remove the camera-latency tail. We deliberately do **not** use onset lag as a decision signal: although a real reflector has near-zero onset, subject motion during the flash confounds it (a head drifting while the flash ramps reads a slow, screen-like onset), so onset is logged for diagnostics only. The probe **abstains** (returns inconclusive, never a spoof verdict) when (i) exposure cannot be locked — the uncontrolled sensor's own auto-gain then mimics a screen's persistence; (ii) the flash rise is below a measurability floor (bright ambient, distant monitor, or a screen already at maximum backlight); or (iii) no face crop is available. This mirrors the capture-quality abstention principle of §4.x: a confident verdict is never issued on input the system could not control.

**Scope and limits.** The probe requires programmatic exposure control (`MediaStreamTrack.applyConstraints` with `exposureMode:'manual'`), which desktop webcams expose but most mobile front cameras do not; on the latter it abstains. It discloses *auto-brightness-enabled* screens — a replay device pinned to a fixed backlight will not ramp or persist and is left to the spatial reflection and planarity signals. A small live validation on a desktop webcam (real face vs. phone video-replay) cleanly separated the two on persistence; a consented at-scale capture for paper-grade APCER/BPCER is future work (§8.5, §9).
